...
class UniqueID(models.Model):
    """
    Abstract base that replaces the default auto-incrementing integer PK
    with a UUID, generated in Python at instantiation time (not on save).
    """
...
class TimeStampedModel(models.Model):
    """
    Abstract base that adds creation/update timestamps and a soft-delete
    marker to a model.

    `deleted_at` is only a timestamp field — it does not filter querysets
    by itself. Models using this mixin are responsible for overriding
    `delete()` to set it and, if needed, providing a manager that excludes
    soft-deleted rows (see `ActiveListingManager` for an example).
    """
